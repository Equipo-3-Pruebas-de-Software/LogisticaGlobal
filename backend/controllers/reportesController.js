...
const { createReporte, readReporte, readAllReportes } = require('../models/reportesModel');

const createReporte = (req, res) => {
  const { nombre, descripcion, reportes } = req.body;

  if (!nombre || !descripcion || reportes.length === 0) {
  ...
  db.beginTransaction((err) => {
    ...
    createReporte({ nombre, descripcion }, (err, reporteId) => {
      if (err) return db.rollback(() => res.status(500).json({ error: 'Error creando reporte' }));
